...
function saveState() {
    window.api.saveData({
        ...
        if (res && res.ok === false) showToast('Failed to save data: ' + res.error, 'error');
    }).catch(e => showToast('Failed to save data: ' + e.message, 'error'));
}
...
async function loadState() {
    try {
        ...
        if (!data || data.version !== DATA_VERSION) return; // no data file yet — start fresh
        if (data.timesheets) state.timesheets = data.timesheets;
        if (data.dailyNotes) state.dailyNotes = data.dailyNotes;
        if (data.pto)        state.pto        = data.pto;
...
function exportExcel() {
    ...
    window.api.exportXlsx(new Uint8Array(wbout), filename).then(res => {
        if (res && res.ok) showToast('Excel file exported', 'success');
        else if (res && res.error) showToast('Export failed: ' + res.error, 'error');
...
function setupEventListeners() {
    ...
    document.getElementById('sidebar-collapse-btn').addEventListener('click', () => {
        ...
        window.api.setSidebarCollapsed(sidebar.classList.contains('collapsed'));
    });
...
async function init() {
    ...
    applySettings();
    saveState();
    if (window.api.getSidebarCollapsed()) {
